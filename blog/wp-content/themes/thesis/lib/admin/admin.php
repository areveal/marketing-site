...
final class thesis_admin {
	public function __construct() {
		...
		add_action('admin_post_thesis_license_key', array($this, 'save_license_key'));
		...
		add_action('admin_post_thesis_fixes', array($this, 'handle_fixes'));
		...
		if (!empty($_GET['canvas'])) {
			if ($_GET['canvas'] === 'system_status')
				add_action('thesis_admin_canvas', array($this, 'system_status'));
			elseif ($_GET['canvas'] === 'license_key')
				add_action('thesis_admin_canvas', array($this, 'license_key'));
			elseif ($_GET['canvas'] === 'fix')
				add_action('thesis_admin_canvas', array($this, 'fix'));
		}
	...
	private function canvas() {
		...
		echo
			...
			"\t\t\t<h3>", __('Getting Started with Thesis', 'thesis'), "</h3>\n",
			"\t\t\t<p>", sprintf(__('If you&#8217;re a new Thesis user, or even if you&#8217;d just like to get acquainted with the latest features and functionality, then I highly recommend the <a href="%s">Getting Started Guide</a>.', 'thesis'), esc_url('http://diythemes.com/thesis/rtfm/getting-started/#section-intro')), "</p>\n",
			"\t\t\t<h4>", __('What&#8217;s New in Version 2.1?', 'thesis'), "</h4>\n",
...
/*			'schema' => array(
	...
	public function menu_fix() {
		if (!empty($_GET['canvas']))
			echo	"<script type=\"text/javascript\">\n",
					...
					"\t\t\tif (/", esc_attr($_GET['canvas']), "(?!\w)/.test($(this).attr('href')))\n",
	...
	public function license_key() {
		...
		echo
			"\t\t<h3>", __('License Key Management', 'thesis'), "</h3>\n",
			...
			"\t\t\t<input type=\"submit\" data-style=\"button delete\" id=\"delete_options\" name=\"delete_options\" value=\"", __('Delete License Key', 'thesis'), "\" />\n",
